...
def main(argv=None):
	...
	parser.add_argument('--output', default='../../three.js')
	...
	if args.minify is False:
		...
		source = ' '.join(sources)
		cmd = 'java -jar ../../compiler.jar --warning_level=VERBOSE --jscomp_off=globalThis --externs %s --jscomp_off=checkTypes --language_in=ECMASCRIPT5_STRICT --js %s --js_output_file %s %s' % (externs, path, output, sourcemapargs)
		os.system(cmd)
